...
def model_evaluation(model, test_data, tokenizer, slot_meta,epoch,slot_ans=None,  op_code='4',
    ...
    for di, i in enumerate(test_data):
        ...
        #     continue
        # i.make_instance(tokenizer, word_dropout=0.,turn=2)
        if i.turn_id == 0:
            last_dialog_state={}
        ...
        for idx,op_pred in enumerate(op_predictions):
                ...
                if isvalid:
                    ...
                    if op_pred==1:
                        ...
                        if input_ids[start_prediction[idx]-1:end_prediction[idx]]==3:
                            slot_idx[idx]=-1
                        elif gen_predictions[idx]==len(slot_ans[idx]['db'])-1:
                            slot_idx[idx]=-1
                        else:
            ...
            if slot_ans_idx[idx] == len(slot_ans[idx]['db']) - 1:
                slot_ans_idx[idx]=-1
            if slot_idx[idx] == len(slot_ans[idx]['db']) - 1:
                slot_idx[idx] = -1
            if cate_mask[idx]:
                cateslot+=1
                ...
                nocateslot+=1
                if slot_idx[idx]!=slot_ans_idx[idx] and (slot_idx[idx]!=3 or gen_ids!=[]) and (slot_idx[idx]==[] or gen_ids!=[3]):
                    iswrong = True
                    isnocatewrong=True
                    is_domain_correct[slot_ans[idx]['name'].split("-")[0]]=False
                else:
